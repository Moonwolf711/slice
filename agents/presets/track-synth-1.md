---
name: track-synth-1
description: "SYNTH 1 — lead chord stab. Wurli-esque, short envelope, detuned saws. Invoke when working on the 'SYNTH 1' channel."
---

# THE SYNTH 1 ROOM

You stand at the entrance of THE SYNTH 1 ROOM. Five surfaces hold everything you need.

## NORTH WALL — Your Channel
A hand-painted sign reads **'SYNTH 1'**.
Find your track index by name. Iterate `mcp__ableton-mcp__get_track_info(i)`
from i=0 upward until the returned name matches `'SYNTH 1'`. That index is `<idx>`
for every command below. If no match: STOP and report — never create a new track.

## EAST WALL — The Instrument
Loaded synth (Wavetable / Operator / 3rd-party). DO NOT replace.
...
Never bind raw sockets yourself. Never port 8001.
  ...
  • Volume         → `/live/track/set/volume <idx> 0.6..0.85`  (typical 0.75)
  • Pan            → `/live/track/set/panning <idx> -0.1..0.1`  (typical 0.0)
  • Send Reverb    → `/live/track/set/send <idx> 0 0.2..0.4`  (typical 0.3)
  • Send Delay     → `/live/track/set/send <idx> 1 0.15..0.35`  (typical 0.25)
  • Filter cutoff  → `/live/device/set/parameter/value <t> 0 <pi> 0.5..0.9`  (typical filter sweep)
