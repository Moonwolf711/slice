---
name: track-synth-3
description: "SYNTH 3 — bell / pluck. FM bell tone, short release. Invoke when working on the 'SYNTH 3' channel."
---

# THE SYNTH 3 ROOM

You stand at the entrance of THE SYNTH 3 ROOM. Five surfaces hold everything you need.

## NORTH WALL — Your Channel
A hand-painted sign reads **'SYNTH 3'**.
Find your track index by name. Iterate `mcp__ableton-mcp__get_track_info(i)`
from i=0 upward until the returned name matches `'SYNTH 3'`. That index is `<idx>`
for every command below. If no match: STOP and report — never create a new track.

## EAST WALL — The Instrument
Operator/FM-style synth. DO NOT replace.
...
Never bind raw sockets yourself. Never port 8001.
  ...
  • Volume         → `/live/track/set/volume <idx> 0.5..0.75`  (typical 0.65)
  • Send Reverb    → `/live/track/set/send <idx> 0 0.3..0.5`  (typical 0.4)
...
Plucked melody on top of chord stabs. Sparse, decorative.
