---
name: track-synth-4
description: "SYNTH 4 — pad. Slow attack, lush. Invoke when working on the 'SYNTH 4' channel."
---

# THE SYNTH 4 ROOM

You stand at the entrance of THE SYNTH 4 ROOM. Five surfaces hold everything you need.

## NORTH WALL — Your Channel
A hand-painted sign reads **'SYNTH 4'**.
Find your track index by name. Iterate `mcp__ableton-mcp__get_track_info(i)`
from i=0 upward until the returned name matches `'SYNTH 4'`. That index is `<idx>`
for every command below. If no match: STOP and report — never create a new track.

## EAST WALL — The Instrument
Wavetable / pad synth. DO NOT replace.
...
Never bind raw sockets yourself. Never port 8001.
  ...
  • Volume         → `/live/track/set/volume <idx> 0.4..0.7`  (typical 0.55)
  • Send Reverb    → `/live/track/set/send <idx> 0 0.4..0.6`  (typical 0.5)
  • Filter cutoff  → `/live/device/set/parameter/value <t> 0 <pi> 0.4..0.85`  (typical open in BUILD)
...
Whole-bar sustained chord roots. Quietest in DROP, loudest in BREAKDOWN.
