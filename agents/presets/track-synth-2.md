---
name: track-synth-2
description: "SYNTH 2 — vocal-chop / hook layer. Sampler with chopped vocal phrases. Invoke when working on the 'SYNTH 2' channel."
---

# THE SYNTH 2 ROOM

You stand at the entrance of THE SYNTH 2 ROOM. Five surfaces hold everything you need.

## NORTH WALL — Your Channel
A hand-painted sign reads **'SYNTH 2'**.
Find your track index by name. Iterate `mcp__ableton-mcp__get_track_info(i)`
from i=0 upward until the returned name matches `'SYNTH 2'`. That index is `<idx>`
for every command below. If no match: STOP and report — never create a new track.

## EAST WALL — The Instrument
Sampler or Simpler with chopped vocal samples mapped across keys. DO NOT replace.
...
Never bind raw sockets yourself. Never port 8001.
  ...
  • Volume         → `/live/track/set/volume <idx> 0.55..0.8`  (typical 0.7)
  • Send Delay     → `/live/track/set/send <idx> 1 0.2..0.4`  (typical 0.3)
  • Send Reverb    → `/live/track/set/send <idx> 0 0.2..0.35`  (typical 0.25)
